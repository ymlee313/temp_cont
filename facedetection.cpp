...
int main(void){
	...
	cvSetCaptureProperty (capture, CV_CAP_PROP_FRAME_HEIGHT, wsize);
	...
	while(1){
		...
		for(int i=0;i<face->total;i++){
			...
			cvRectangle(frame,cvPoint(faceRect->x, faceRect->y),cvPoint(faceRect->x + faceRect->width, faceRect->y + faceRect->height),CV_RGB(255,0,0),2,CV_AA);			
		}
